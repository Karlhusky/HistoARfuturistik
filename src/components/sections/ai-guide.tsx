...
const seed = [
  ...
  {
    role: "ai",
    text: "Founded in the 3rd century BCE, it housed up to 400,000 scrolls. Want me to open the AR reconstruction?",
  },
];
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {prompts.map((p) => (
              <button
                key={p}
                onClick={() => send(p)}
                className="rounded-full glass px-4 py-2 text-xs text-muted-foreground transition hover:bg-white/10 hover:text-foreground"
              >
                {p}
              </button>
            ))}
